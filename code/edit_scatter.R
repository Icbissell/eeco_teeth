...
print(tseries::adf.test(d18O_IAR$IAR)) # P-value too high to reject null
acf(d18O_IAR$IAR) # some indicators of non-stationarity 

print(tseries::adf.test(d18O_IAR$d18O)) # P-value too high to reject null
acf(d18O_IAR$d18O) # some indicators of non-stationarity 
...
print(tseries::adf.test(diff(d18O_IAR$d18O))) #p-value increases
acf(diff(d18O_IAR$d18O)) #acf looks better

## tried for second order but looks even worse

#try correction with linear model fit:
model1 <- lm(d18O_IAR$IAR ~ seq_along(d18O_IAR$IAR))
model2 <- lm(d18O_IAR$d18O ~ seq_along(d18O_IAR$d18O))
...
## compute for diff time series
print(tseries::adf.test(series1_detrend)) #p-value increases
acf(series1_detrend) #acf looks better

print(tseries::adf.test(series2_detrend)) #p-value increases
acf(series2_detrend) #acf looks better

summary(lm(diff(log(d18O_IAR$IAR)) ~ diff(d18O_IAR$d18O)))
